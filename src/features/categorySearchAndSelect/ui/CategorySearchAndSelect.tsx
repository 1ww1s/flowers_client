...
import classes from './categorySearchAndSelect.module.scss'
...
export const CategorySearchAndSelect: FC<IProps> = ({setSelected}) => {
    ...
    const getList = async () => {
        try{
            setIsLoading(true)
            const name = await categoryService.getStartsWith(value)
            setValues(name)
        }
        catch(e){
    ...
    return (
        <section className={classes.searchAndSelectProduct}>
            <Autocomplete 
                value={value}
                setValue={setValue}
                sign="Начните вводить имя и выберите из списка нужный продуку"
