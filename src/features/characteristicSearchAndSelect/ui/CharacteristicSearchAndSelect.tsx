...
export const CharacteristicSearchAndSelect: FC<IProps> = ({setSelected}) => {
    ...
    const getList = async () => {
        try{
            setIsLoading(true)
            const names = await characteristicService.getStartsWith(value)
            setValues(names)
        }
        catch(e){
    ...
    return (
        <section className={classes.searchAndSelectProduct}>
            <Autocomplete 
                value={value}
                setValue={setValue}
                sign="Начните вводить имя и выберите из списка нужную характеристику"
